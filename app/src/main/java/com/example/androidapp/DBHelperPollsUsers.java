...
public class DBHelperPollsUsers extends SQLiteOpenHelper {

    public static final String DBName = "Login.db";

    public DBHelperPollsUsers(Context context) {
        super(context, "DataPollsUsers.db", null, 1);
    }
    ...
    public Boolean insertDataForPollsQuestion(String id, String nameUser, String nameQuestion, String answer, String date,String pollId, String longitude,String latitude) {
    ...
    public  Cursor readAllDataFromPollsUsers(String pollId, String userName){
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = null;
        if(db != null){
            cursor = db.rawQuery("SELECT * FROM " + "polls_question" + " where nameUser = '" + userName + "'" + "and pollId = '" + pollId + "'"  , null);
        }
        return cursor;
    }
    public Cursor readAllDataFromPollsByIdResults(String id){
        String query = "SELECT * FROM polls";
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = null;
        if(db != null){
            cursor = db.rawQuery("SELECT * FROM " + "polls_question" + " where pollId = '" + id + "'" , null);
        }
        return cursor;
    }
    public Cursor readAllDataByUserName(String userName){
        String query = "SELECT * FROM questions WHERE pollId=id";
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = null;
        if(db != null){
            cursor = db.rawQuery("SELECT * FROM " + "polls_question" + " where nameUser = '" + userName + "'" , null);
        }
        return cursor;
    }
